...
*Verified July 2026:* both zombie listings are still live under the original author's account — AMO at v1.6.3 with **~119 daily users and a 5-star rating** (real, reachable users worth migrating carefully), CWS at v1.6.3 with ~595 nominal users (all dark since the MV2 shutdown). A name-collision sweep found **no third-party product named "VTT Bridge"** on either store, GitHub, npm, or trademark databases — the name is ours to keep; the only search-result neighbor a new listing would have is the old listing itself, so plan its retirement (transfer, delisting request, or a clear "v2 has moved" note) as part of the launch.
